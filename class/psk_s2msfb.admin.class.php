...
class PSK_S2MSFBAdmin
{
	...
	*/
	...
	public static function get_admin_screen_title($method) {
		...
		if ($parent!='') $parent = $parent['name'].' &gt; '.$parent['chil'][$child]['name'];
		return $parent;
	}
	...
	*/
	...
	{
		// Define actions
		add_action("ws_plugin__s2member_during_add_admin_options_additional_pages"	, array(__CLASS__,'admin_menu_items'), 666);
	...
	public static function init_assets()
	{
		if (self::get_admin_my_plugin_page() != '') {
			...
			wp_enqueue_script(PSK_S2MSFB_ID.'.admin', PSK_S2MSFB_JS_URL . 'admin.js', array('jquery'), false,false); // in header because of alert manager
	        wp_enqueue_style(PSK_S2MSFB_ID.'.admin', PSK_S2MSFB_CSS_URL . 'admin.css' );
		}
	}
	...
	public static function get_admin_header($method)
	{
		...
		foreach (self::$admin_menu as $id=>$pos) {
			if      ($id=='left')  $innerul=' class="nav" role="navigation"';
			else if ($id=='right') $innerul=' class="nav pull-right"';
			else                   $innerul=' class="nav" role="navigation"';
			$menu.='<ul'.$innerul.'>';
			foreach ($pos as $pid=>$parent) {
				$pright = (isset($parent['right'])) ? $parent['right'] : self::$admin_menu_right;
				if ( PSK_Tools::current_user_cans( $pright ) ) {
					...
					foreach ($parent['chil'] as $cid=>$child) {
						...
						else if ($child=='')  $menu.= '<li class="divider"></li>';
						else                  $menu.= '<li class="nav-header">'.$child.'</li>';
					}
		            $menu.='    </ul>';
		            $menu.='  </li>';
		        }
