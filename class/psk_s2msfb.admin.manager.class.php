...
class PSK_S2MSFBAdminManager {
	...
	public static function init_shortcode_options() {
		self::$shortcode_options = array(
			...
			array(
				...
				'more'     => __( 'Can be set to <code>1</code> to display files without path', PSK_S2MSFB_ID ) . '<br/>' .
	...
	public static function admin_screen_manager_cache() {
		...
		echo '<br/><br/>';
		echo __( 'If you find that something is odd with your files, select the hash option below to re-compute md5 hash for all files. Be careful, it can be very very long!', PSK_S2MSFB_ID );
		echo '<br/><br/>';
		...
		$count = (int) get_option( PSK_S2MSFB_DB_FILES_CLEAN_COUNT_OPT );
		...
		if ( $count == 0 ) {
			...
			$count = sprintf( __( '%s files found', PSK_S2MSFB_ID ), $count );
		}
		...
		echo '&nbsp;&nbsp;&nbsp;<input type="checkbox" value="1" name="hash" id="hash" />';
		echo '<span class="help-inline"><em>' . __( 'Compare files with md5 hash, it can be very long...' ) . '</em></span>';
