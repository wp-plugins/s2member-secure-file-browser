...
!function ($) {
	...
	Modal.prototype = {constructor: Modal, toggle: function () {
		...
		this.$element.trigger(e), this.isShown || e.isDefaultPrevented() || (this.isShown = !0, this.escape(), this.backdrop(function () {
			...
			that.$element.parent().length || that.$element.appendTo($('div.psk')), that.$element.show(), transition && that.$element[0].offsetWidth, that.$element.addClass("in").attr("aria-hidden", !1), that.enforceFocus(), transition ? that.$element.one($.support.transition.end, function () {
